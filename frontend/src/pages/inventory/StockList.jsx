...
export default function StockList() {
  ...
  const columns = type === 'bahan' ? [
    ...
    { label: 'Stok Masuk', render: (r) => <span className="text-emerald-600 font-medium">{r.stok_masuk}</span> },
    { label: 'Stok Keluar', render: (r) => <span className="text-red-600 font-medium">{r.stok_keluar}</span> },
    { label: 'Stok Saat Ini', render: (r) => <span className="font-bold text-wash-800">{r.stok_saat_ini}</span> },
  ] : [
  ...
  return (
    <div>
      <PageHeader title="Stok Pusat" breadcrumbs={[{ label: 'Home', to: '/' }, { label: 'Inventory' }, { label: 'Stok' }]} />
